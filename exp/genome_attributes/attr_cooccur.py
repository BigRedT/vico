...
def classifier_attr_attr_reps(model,exp_const):
...
def compute_attr_cooccur(model,dataloader,exp_const):
    num_classes = len(dataloader.dataset.sorted_attribute_synsets)
    rep_dim = model.net.resnet_layers.fc.weight.size(1)

    print('Allocating memory for storing attr cooccur ...')
    attr_cooccur = np.zeros([num_classes,num_classes])
    ...
    for it,data in enumerate(tqdm(dataloader)):
        ...
        prob = sig(logits).data.cpu().numpy()
        for b in range(logits.shape[0]):
            ...
            for attr in on_attrs:
                ...
                attr_cooccur[i] = attr_cooccur[i] + prob[b]
        
    attr_cooccur = attr_cooccur / (num_imgs_per_class[:,None]+1e-6)

    print('Saving attr cooccurrence and related files ...')
    np.save(os.path.join(exp_const.exp_dir,'cooccur.npy'),attr_cooccur)
    np.save(
...
def main(exp_const,data_const,model_const):
    ...
    print('Attr Coocur ...')
    compute_attr_cooccur(model,dataloader,exp_const)

    # print('Classifier Based Attr-Attr Reps ...')
    # classifier_attr_attr_reps(model,exp_const)