...
def classifier_entity_entity_reps(model,exp_const):
...
def compute_entity_entity_reps(model,dataloader,exp_const):
    num_classes = len(dataloader.dataset.wnids)
    rep_dim = model.net.resnet_layers.fc.weight.size(1)

    print('Allocating memory for storing entity-entity reps ...')
    ...
    print('Saving entity-entity reps and related files ...')
